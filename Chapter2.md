...
#### 三个时间属性
* 文件的_Access time_（访问时间）也就是`atime`，是在读取或执行文件时更改的
* 文件的_Modified time_（修改时间）也就是`mtime`，是写入文件时随文件内容的更改而更改的
* 文件的_Change time_（改变时间）也就是`ctime`，是在写入文件、更改所有者、权限或链接设置时随inode的内容更改而更改的。
* `stat`命令可查看文件的`atime`、`mtime`、`ctime`
* 一般而言，`atime`更改了`ctime`未必会变，`mtime`更改`ctime`一定会变。使用`touch`指令可同时改变文件的三个时间。

#### 软链接和硬链接
两种链接的区别在于inode  
* 硬链接直接使用与源文件相同的inode，硬链接文件直接链接到文件放置的块区域，删除源文件不影响硬链接文件的使用。  
硬链接有几个__限制__  
（1）不能跨文件系统，因为不同文件系统有不同的inode table；  
（2）不能链接目录；  
（3）不能跨卷
* 软链接是建立一个独立的文件指向源文件，链接文件大小与源文件路径大小有关（绝对路径与相对路径生成的软链接文件大小不等），删除源文件后链接文件还在但是会提示无法访问。
* 语法`ln [-s] [源文件] [目标文件]`  
不加`-s`就是硬链接，否则为软链接

#### 用户名文件
`/etc/passwd`
该文件内容每行由`：`分割成7个字段，每个字段的具体含义依次是  
* 用户名
* 账号口令（基于安全因素，目前存放至`/etc/shadow`，原位置用`x`代替）
* `uid`用户标识号
* `gid`组标识号
* 注释说明，通常记录用户的一些属性，没有实际意义
* 用户的家目录
* 用户的shell，默认是`/bin/bash`，若想建立一个账号不让其登录，可把字段改成`/sbin/nologin`

#### 密码文件
`/etc/shadow`，该文件用`：`分割成9个字段，每个字段的含义依次是
* 用户名
* 用户密码，开头两个`$`中的数字id表示不同加密方法，CentOS6默认是SHA-512，若显示`！`则表示未设定密码
* 上次更改密码的日期，值是从1970年1月1日到上次更改密码的天数
* 要过多少天才可以更改密码，默认为0，即不限制
* 密码多少天后到期，即多少天内必须更改密码，默认为99999，近似永远不需更改
* 密码到期前的警告期限，若设置为7，则标识当7天后密码到期，系统会发出警告
* 账号失效期限。若设为3，即表示密码已到期，用户未在到期前修改密码则再过3天账号锁定
* 账号生命周期。按距离1970年1月1日天数计算，意为账号在此日期后作废
* 保留用，无意义

#### 增加和删除用户组
* `groupadd`新增用户组  
`group [-g GID] groupadd`，`-g`选项可自定义gid，不加`-g`选项则按照系统默认gid创建组
* `groupdel`删除组  
当组中包含账户时，会提示无法删除，只有移除账户后才能删除该组

#### 增加和删除用户
* `useradd`增加用户  
语法`useradd [-u UID] [-g GID] [-d HOME] [-M] [-s]`  
`-u`自定义UID  
`-g`使其属于某个已存在的组，后面加组名或组id，跟不存在的gid会报错  
`-d`自定义用户的家目录  
`-M`不建立家目录  
`-s`自定义shell  
`useradd`不加任何选项直接跟用户名，则会创建一个与用户名同名的组
* `userdel`删除用户  
语法`userdel [-r] username`  
参数`-r`作用为删除账户时连带该账户的家目录一起删除
